...
def detectHorizontal(positions, rotations, moveThreshold=1.5): 
    ...
    fitsCriteria = (diffAcrossFrames[0] >= moveThreshold and diffAcrossFrames[1] < moveThreshold and diffAcrossFrames[2] < moveThreshold)
    return fitsCriteria
...
def detectRotateSide(positions, rotations, moveThreshold=math.pi / 2): 
...
def test(filename): 
    ...
    for detectorName, fn in detectors.items(): 
        ...
        for i in range(0, len(positionFrames) - framesToEvaluate): 
            ...
            continue
            isHorizontal = detectHorizontal(positionFrames[i:i + 80], rotationFrames[i:i + 80])
            if isHorizontal: 
                print("Is horizontal")

test('data/horizontal-draw.txt')
test('data/raise-wand.txt')
